#!/usr/bin/env bash
# Azuma launcher — local (SQLite, default) or docker (PostgreSQL) mode.
set -euo pipefail
...
TOOL="azuma"
BACKEND_PORT="${AZUMA_BACKEND_PORT:-8030}"
FRONTEND_PORT="${AZUMA_FRONTEND_PORT:-4230}"
DB_PATH="${AZUMA_DB_PATH:-$ROOT_DIR/backend/azuma.db}"
XWA_SDK_LOCAL="${XWA_SDK_DIR:-/home/x/Documents/xwebanalysis/xwa-sdk/bindings/python}"
...
usage() {
    cat <<EOF
Usage: ./azuma.sh [local|docker] [backend|frontend|all]
