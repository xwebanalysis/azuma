...
JWT_SECRET = os.getenv("AZUMA_JWT_SECRET") or None
AUTH_PASSWORD = os.getenv("AZUMA_AUTH_PASSWORD", "azuma")
TOKEN_TTL_HOURS = 24

RATE_LIMIT_MAX = int(
    os.getenv("AZUMA_RATE_LIMIT_MAX") or os.getenv("XWA_RATE_LIMIT_MAX") or "120"
)
...
def issue_token() -> str:
    now = int(time.time())
    return pyjwt.encode(
        {"sub": "azuma-user", "iat": now, "exp": now + TOKEN_TTL_HOURS * 3600},
        JWT_SECRET,
        algorithm="HS256",
    )
